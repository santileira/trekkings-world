...
export default function ShareButtons({ url, title, description, locale }: ShareButtonsProps) {
  ...
  const copyLabel = locale === 'es' ? 'Copiar link' : 'Copy link';
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-black hover:bg-gray-800 text-white transition-colors"
        title="X (Twitter)"
      >
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="relative">
        <button
          ...
          className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-slate-200 hover:bg-slate-300 text-slate-600 transition-colors"
          title={copyLabel}
        >
          {copied ? (
